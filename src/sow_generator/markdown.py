...
def build_toc(
    ...
    for story in story_config:
        ...
        toc_lines.append(f"  - [{epic_heading}](#{slugify_heading(epic_heading)})")
        toc_lines.append(f"    - [{feature_heading}](#{slugify_heading(feature_heading)})")
    toc_lines.extend(
        [
...
def generate_sow(data: dict[str, Any]) -> str:
    ...
    lines.extend(["", "## Project Overview", "", project["description"], "", "## Team Structure", ""])
    lines.extend(
        build_table(
            ["Role", "Name", "Email", "Phone"],
            [
                [str(member["role"]), str(member["name"]), str(member["email"]), str(member["phone"])]
                for member in team["members"]
            ],
        )
    )
    lines.extend(["", "## Methodology: Technical Kanban", "", str(kanban["description"]), "", "### Kanban Board Columns"])
    lines.extend(
        build_table(
            ["Key", "Description"],
            [[str(column["key"]), str(column["description"])] for column in kanban["columns"]],
        )
    )
    lines.extend(["", "### Key Principles"])
    lines.extend(
        build_table(
            ["Key", "Description"],
            [[str(principle["key"]), str(principle["description"])] for principle in kanban["principles"]],
        )
    )
    ...
    for story in story_config:
        ...
        lines.extend(
            build_table(
                ...
                [[f"AC-{index:02d}", epic["id"], str(criterion)] for index, criterion in enumerate(story["ac"], start=1)],
            )
        )
        lines.extend([""])
    ...
    lines.extend(
        build_table(
            ...
            [
                [deliverable["id"], deliverable["name"], deliverable["description"], deliverable["access_details"]]
                for deliverable in deliverables
            ],
        )
    )
    lines.extend(["", "### Delivery Estimate Summary"])
    lines.extend(
        build_table(
